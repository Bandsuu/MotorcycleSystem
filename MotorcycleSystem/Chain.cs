...
namespace MotorcycleSystem
{
    public partial class Chain : Form
    {
        ...
        {
            string Product = "Motorcycle Chain";
            string Price = "550";
        ...
        private void SaveData(string Product, string Price, string quantity, string name, string address, DateTime orderTime)
        {
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                {
                    conn.Open();
                    string query = "INSERT INTO chain_tbl (Product, Price, Quantity, Name, Address, OrderTime) VALUES (@Product, @Price, @Quantity, @Name, @Address, @OrderTime)";
