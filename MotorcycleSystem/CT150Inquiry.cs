...
{
    public partial class CT150Inquiry : Form
    {
        ...
        private void SaveData(string model, string price, string downpayment, string name, string age, string gender, string nationality, string occupation, string contactnumber, string address)
        {
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                {
                    conn.Open();
                    string query = "INSERT INTO ct_tbl (Model, Price, Downpayment, Name, Age, Gender, Nationality, Occupation, ContactNumber, Address) VALUES (@Model, @Price, @Downpayment, @Name, @Age, @Gender, @Nationality, @Occupation, @ContactNumber, @Address)";
