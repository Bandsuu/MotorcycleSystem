...
namespace MotorcycleSystem
{
    public partial class GravisInquiry : Form
    {
        ...
        {
            Gravis_125 gravis_125 = new Gravis_125();
            gravis_125.Show();
            this.Close();
        }
        ...
        private void SaveData(string model, string price, string downpayment, string name, string age, string gender, string nationality, string occupation, string contactnumber, string address)
        {
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                {
                    conn.Open();
                    string query = "INSERT INTO gravis_tbl (Model, Price, Downpayment, Name, Age, Gender, Nationality, Occupation, ContactNumber, Address) VALUES (@Model, @Price, @Downpayment, @Name, @Age, @Gender, @Nationality, @Occupation, @ContactNumber, @Address)";
